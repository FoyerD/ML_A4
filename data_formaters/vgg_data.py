...
def write_yolo_label(label_path, class_id):
    with open(label_path, 'w') as f:
        f.write(f"{class_id}\n")  # Only one label per image for classification, and bounding box is the whole image
...
def prepare_dataset(X, y, split):
    for image_filename, label_class in zip(X, y):
        ...
        dst_image_path = os.path.join(images_base_dir, split, image_filename)
        shutil.copy(src_image_path, dst_image_path)

        label_filename = image_filename.replace('.jpg', '.txt')
        # Create YOLO label file
        label_filepath = os.path.join(labels_base_dir, split, label_filename)
